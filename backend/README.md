...
四种聊天模式均已接入实际工作流。论文精读不会自动猜测目标论文，请求必须显式传入 `paper_id`。
...
研究诊断模式会收集当前项目内已解析论文证据，生成 `research_diagnosis` Artifact。诊断输出会区分文献证据支持的判断、基于用户材料的合理推断、待补充证据、风险和下一步问题。
